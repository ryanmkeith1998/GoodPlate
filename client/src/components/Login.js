  ...
  function signup_enter() {
    signup()
      .then(({data}) => {
        ...
        console.log(data);
        //set_info([data.signup.user.email, data.signup.user.name]);
      })
      .catch(e => {
        console.log(e);
        (formState.error === 1) ? alert("Email is already in use!") : alert("Make sure to fill every input field!")
        setFormState({
          login: 1,
          email: '',
          password: '',
  ...
  return (
    <div class="whole-login">
      ...
      <div>
        ...
        {(formState.login === 2) && (
          <p id='location-header'> Welcome back {formState.name}! </p>
        )}
      </div>
